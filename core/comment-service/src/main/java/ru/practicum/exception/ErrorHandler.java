...
public class ErrorHandler {
    ...
    public ApiError handleServiceException(final ServiceException e) {
        log.error("500 Service error: {}", e.getMessage(), e);
        return ApiError.builder()
                .status(HttpStatus.INTERNAL_SERVER_ERROR.toString())
                .reason("Ошибка при вызове внешнего сервиса")
                .message(e.getMessage())
                .timestamp(LocalDateTime.now().format(FORMATTER))
                .build();
    }
    ...
    public ApiError handleFeignException(final FeignException e) {
        log.error("500 Feign error: {}", e.getMessage(), e);
        return ApiError.builder()
                .status(HttpStatus.INTERNAL_SERVER_ERROR.toString())
                .reason("Ошибка при вызове внешнего сервиса")
                .message("Сервис временно недоступен: " + e.getMessage())
                .timestamp(LocalDateTime.now().format(FORMATTER))
                .build();
    }
}